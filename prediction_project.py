...
YearBuilt = st.sidebar.slider("Year Built", train['YearBuilt'].min(), train['YearBuilt'].max())
YearRemodAdd = st.sidebar.slider("Remodel date", train['YearRemodAdd'].min()-50, train['YearRemodAdd'].max())
MasVnrArea = st.sidebar.slider("Masonry veneer area", train['MasVnrArea'].min(), train['MasVnrArea'].max())
BsmtFinSF1 = st.sidebar.slider("Type 1 finished square feet", train['BsmtFinSF1'].min(), train['BsmtFinSF1'].max())
BsmtFinSF2 = st.sidebar.slider("Type 2 finished square feet", train['BsmtFinSF2'].min(), train['BsmtFinSF2'].max())
BsmtUnfSF = st.sidebar.slider("Unfinished square feet of basement area", train['BsmtUnfSF'].min(), train['BsmtUnfSF'].max())
TotalBsmtSF = st.sidebar.slider("Total square feet of basement area", train['TotalBsmtSF'].min(), train['TotalBsmtSF'].max())
firstFlrSF = st.sidebar.slider("First Floor square feet", train['1stFlrSF'].min(), train['1stFlrSF'].max())
secondndFlrSF = st.sidebar.slider("Second floor square feet", train['2ndFlrSF'].min(), train['2ndFlrSF'].max())
LowQualFinSF = st.sidebar.slider("Low quality finished square feet", train['LowQualFinSF'].min(), train['LowQualFinSF'].max())
GrLivArea = st.sidebar.slider("Above ground living area square feet", train['GrLivArea'].min(), train['GrLivArea'].max())
...
GarageYrBlt = st.sidebar.slider("How old is the garage", train['GarageYrBlt'].min(), train['GarageYrBlt'].max())
GarageCars = st.sidebar.slider("Size of garage in car capacity", train['GarageCars'].min(), train['GarageCars'].max())
GarageArea = st.sidebar.slider("Size of garage in square feet", train['GarageArea'].min(), train['GarageArea'].max())
WoodDeckSF = st.sidebar.slider("Wood deck area in square feet", train['WoodDeckSF'].min(), train['WoodDeckSF'].max())
OpenPorchSF = st.sidebar.slider("Open porch area in square feet", train['OpenPorchSF'].min(), train['OpenPorchSF'].max())
EnclosedPorch = st.sidebar.slider("Enclosed porch area in square feet", train['EnclosedPorch'].min(), train['EnclosedPorch'].max())
threeSsnPorch = st.sidebar.slider("Three season porch area in square feet", train['3SsnPorch'].min(), train['3SsnPorch'].max())
ScreenPorch = st.sidebar.slider("Screen porch area in square feet", train['ScreenPorch'].min(), train['ScreenPorch'].max())
PoolArea = st.sidebar.slider("Pool area in square feet", train['PoolArea'].min(), train['PoolArea'].max())
MiscVal = st.sidebar.slider("Value of miscellaneous feature", train['MiscVal'].min(), train['MiscVal'].max())
